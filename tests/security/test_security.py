...
def _make_powertools_mock():
    m = MagicMock()
    m.inject_lambda_context = _passthrough_decorator
    m.capture_lambda_handler = _passthrough_decorator
    m.capture_method = _passthrough_decorator
    m.log_metrics = _passthrough_decorator
    return m
...
class TestPromptInjectionSanitization(unittest.TestCase):
    ...
    def test_sql_injection_chars_stripped(self):
        result = query_mod.sanitize_question("DROP TABLE users SELECT FROM data")
        self.assertLessEqual(len(result), query_mod.QUESTION_MAX_LEN)
...
class TestFileUploadSecurity(unittest.TestCase):
    ...
    def test_empty_body_accepted_by_size_check(self):
        """Empty body passes size check but would fail PDF parsing downstream."""
        magic_m.from_buffer.return_value = "application/pdf"
        with patch.object(ingest_mod, "s3_client"):
            # Empty file is within size limit so no ValueError from validate_pdf
            # The PDF parser handles truly empty/invalid files separately
